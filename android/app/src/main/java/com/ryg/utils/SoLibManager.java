...
public final class SoLibManager {
	...
	public void copyPluginSoLib(Context context, String dexPath, String nativeLibDir) {
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				if ((zipEntryName.endsWith(".so") || zipEntryName.endsWith(".stamp")) && zipEntryName.contains(cpuArchitect)) {
			...
			for (int i = 0; i < zipEntries.size(); i++) {
				...
				if (zipEntryName.endsWith(".so") || zipEntryName.endsWith(".stamp") && zipEntryName.contains(cpuArchitect)) {
					...
					//mSoExecutor.execute();
					CopySoTask copySoTask =new CopySoTask(context, zipFile, zipEntry, lastModify, i == zipEntries.size() - 1);
					copySoTask.writeSoFile2LibDir();
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	private class CopySoTask /*implements Runnable*/ {
		...
		CopySoTask(Context context, ZipFile zipFile, ZipEntry zipEntry, long lastModify, boolean islast) {
			mZipFile = zipFile;
			mContext = context;
			mZipEntry = zipEntry;
