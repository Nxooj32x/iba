...
import 'package:iba/src/list/state.dart';
...
class _ListConnector extends ConnOp<ListState, List<ItemBean>> {
  ...
  List<ItemBean> get(ListState state) {
    if (state.items?.isNotEmpty == true) {
      return state.items.map<ItemBean>((ItemState data)=> ItemBean("MyItem",data) ).toList(growable: true);
    }
    return <ItemBean>[];
  }
  ...
  void set(ListState state, List<ItemBean> items) {
    if (items?.isNotEmpty == true) {
      state.items = List<ItemState>.from(items.map<ItemState>((ItemBean bean)=> bean.data).toList());
    } else {
